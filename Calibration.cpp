...
Calibration::Calibration() {
  ...
  washThreshold = 20.0;  // 초기 기본값
  spinThreshold = 100.0; // 초기 기본값
}
...
void Calibration::runCalibrationCycle(VibrationSensor &sensor) {
  ...
  while (millis() - startTime < CALIBRATION_DURATION_MS) {
    ...
    if (data.magnitude > 1.0) { // 의미 있는 진동만 데이터에 포함
  ...
  // 기준점 및 '진동 지문' 최종 계산
  spinThreshold = maxVibration * 0.7; // 탈수 기준점 (최대 진동값에서 살짝 여유를 둔 0.7을 곱한 값)
  
  // 0으로 나누는 오류 방지
  if (vibrationCount > 0) {
    float avgVibration = totalVibration / vibrationCount; // 평균 진동 값 (전체 진동에서 횟수로 나누기)
    washThreshold = avgVibration * 1.5; // 세탁 기준점 (세탁 중에도 갑자기 진동이 튈수도 있어서 1.5 곱해주기)
  }
